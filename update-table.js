...
cPage = cPage.replace(/<td>\n\s*\{customer\.service \? services\.find\(s => s\.id === customer\.service\)\?\.name \|\| 'Unknown' : 'None'\}\n\s*<\/td>/,
  `$&
                  <td>
                    {customer.service_type && customer.service_type.length > 0 
                      ? customer.service_type.map((id: string) => docServices.find(d => d.id === id)?.name).filter(Boolean).join(', ') 
                      : 'None'}
                  </td>`);
